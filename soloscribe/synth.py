...
# attack, and never below 70 ms (which is a short 16th at any tempo a solo is
# played at). Without it the reading is audibly more legato than the playing.
_ARTIC_GAP_FRACTION = 0.85
_ARTIC_MIN_SEC = 0.07

# Matching windows for the two calibrations. 60 ms for the onset convention is
# the brief's; 150 ms for the render lag is loose enough to survive a synth
# latency several times the ~28 ms measured here while still throwing out a
# note that produced no onset at all and matched something a bar away.
...
def render_events_fluid(
    ...
    clip rather than assumed:
      ...
      render lag    fluidsynth emits late. Median over (nearest detected onset
                    ...
                    150 ms. Measured 28.0 ms on this machine (fluidsynth 2.5.7,
                    MS Basic.sf3, 8-note line, identical across three renders);
                    the demo clip that motivated this measured 32–36 ms and
                    moved between renders, which is exactly why it is measured
                    and not a constant.
      convention    basic-pitch reports onsets early of where a listener hears
                    ...
                    audio − nearest event start), matched within 60 ms; ~28 ms
                    on the demo clip, +16.4 ms against a KS comparison whose
                    attacks are exactly on the event starts (that residue is
                    the detector's own bias, and it is supposed to survive —
                    it is present in the render lag too and cancels).
